<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="访客IP">
        <template slot-scope="scope">
          <span>{{ scope.row.userIP }}</span>
        </template>
      </el-table-column>
      <el-table-column label="访客所在地">
        <template slot-scope="scope">
          <span>{{ scope.row.userCity }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :close-on-click-modal="false" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="40%">
      <el-form ref="dataForm" :model="temp" size="medium" label-position="left" label-width="100px" style="padding: 0 80px 0 40px;">
        <el-form-item label="日期" prop="date">
          <el-date-picker type="datetime" placeholder="请选择日期" v-model="temp.date" style="width: 100%;"></el-date-picker>
        </el-form-item>
        <el-form-item label="留言" prop="content">
          <el-input v-model="temp.content" placeholder="请输入留言"/>
        </el-form-item>
        <el-form-item label="访客IP" prop="userIP">
          <el-input v-model="temp.userIP" type="textarea" rows="3" placeholder="请输入访客IP"/>
        </el-form-item>
        <el-form-item label="访客所在地" prop="userCity">
          <el-input v-model="temp.userCity" placeholder="请输入访客所在地"/>
        </el-form-item>
        <el-form-item label="头像" prop="avatar">
          <el-input v-model="temp.avatar" placeholder="请输入头像地址"/>
        </el-form-item>
      </el-form>
...
<script>
import { fetchList, createWhisper, updateWhisper, deleteWhisper } from '@/api/whisper'
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.temp.date = parseTime(this.temp.date,'{y}-{m}-{d} {h}:{i}:{s}')
          createWhisper(this.temp).then(() => {
            this.getList()
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData.date = parseTime(tempData.date,'{y}-{m}-{d} {h}:{i}:{s}')
          updateWhisper(tempData).then(() => {
            this.getList()
            this.dialogFormVisible = false
            this.$notify({
              title: '成功',
